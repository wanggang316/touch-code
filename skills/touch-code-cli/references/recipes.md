...
## Run an agent in a side pane _(planned; exec plan 0002)_

```bash
tc pane split right -- claude
```

Opens `claude` in a pane to the right of the current one. Use `tc agent install-hook
claude` _(planned; exec plan 0003)_ beforehand so its completion events surface as
notifications.

## Broadcast a command to every Pane in a Tab _(planned; exec plan 0003)_

```bash
tc broadcast --tab 1/2/1/3 'pwd'
```

Each Pane in Tab `1/2/1/3` receives `pwd\n` exactly as if you typed it. Use `--raw`
...
```bash
./deploy.sh && tc pane notify --body "deploy complete"
```

Invert the boolean to fire on failure:

```bash
./deploy.sh || tc pane notify --body "deploy failed"
```
...
## Capture Pane scrollback to a file _(planned; exec plan 0002)_

```bash
tc pane capture --scope scrollback --lines 500 > /tmp/scrollback.txt
```
