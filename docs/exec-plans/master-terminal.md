...
2. **`tc` quick reference** — flat list of the command groups documented at `apps/mac/tc/TouchCodeCLI.swift:15` (`status`, `launch`, `doctor`, `open`, `tree`, `project`, `worktree`, `tab`, `pane`, `send`, `broadcast`) with their headline subcommands. One line per command.
