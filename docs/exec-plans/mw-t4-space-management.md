...
- **D2** (planning, 2026-04-21): Sheet state lives on **`RootFeature.@Presents spaceManagerSheet`**, mirroring `settingsSheet`. The sheet is window-modal and its effects span features (catalog mutation, terminal panel close via cascade), so it does not belong to `HierarchySidebarFeature`.
