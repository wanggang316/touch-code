...
- **DEC-11 (M4, 2026-04-20): `SplitTree<PaneID>.Tab` → `TouchCodeCore.Tab` qualification.** SwiftUI's `Tab` type (added in macOS 14) shadows `TouchCodeCore.Tab`. Feature views must fully qualify as `TouchCodeCore.Tab` in every signature that mentions the type. Domain name kept — renaming to avoid collision isn't worth the churn.
...
- `apps/mac/touch-code/Tests/RootFeatureTests.swift` — 3 tests: `onLaunchReceivesEngineEventThenCancels` (TestStore with injected AsyncStream continuations; exhaustivity `.off` because the selection subscription is co-running), `selectionChangedUpdatesState`, `lastEventMarkerCoversAllVariants`.

**Verification:** `make mac-build` → `BUILD SUCCEEDED`. `make mac-lint` → clean. `xcodebuild test -scheme touch-code` → **43 tests in 8 suites pass**. App binary launches, shows loading spinner briefly, then the two-column NavigationSplitView with sidebar placeholder + "Select a Worktree" detail. No segfault; `.task { store.send(.onLaunch) }` fires both subscriptions; `.onDisappear` cancels via `onQuit`.
...
**What landed:**
- `apps/mac/touch-code/App/Features/Root/RootFeature.swift` — `SidebarMode` enum, `sidebar: HierarchySidebarFeature.State` substate, `Scope` composition, `.sidebarModeChanged` + `.sidebar(…)` actions.
- `apps/mac/touch-code/App/Features/HierarchySidebar/HierarchySidebarFeature.swift` — @Reducer holding `expandedSpaceIDs` + `expandedProjectIDs` view state; row-tap actions forward to `hierarchyClient.select*`; `pruneExpansionSets` keeps the sets tidy after catalog mutations.
- `apps/mac/touch-code/App/Features/HierarchySidebar/HierarchySidebarView.swift` — `List(.sidebar)` with nested `DisclosureGroup`s (Space → Project → Worktree); row highlight backed by `currentSelection: HierarchySelection` passed from `RootFeature.State`. Reads `HierarchyManager.catalog` via `@Environment` at render time — no catalog copy in TCA state.
- `apps/mac/touch-code/App/Features/InboxSidebar/InboxSidebarPlaceholder.swift` — C6 M5 drop-in slot.
- `apps/mac/touch-code/App/ContentView.swift` — sidebar column now `@ViewBuilder`-switches on `store.sidebarMode`; toolbar segmented Picker toggles between `.hierarchy` and `.inbox`. `.onChange(of: store.selection)` dispatches `.sidebar(.pruneExpansionSets(…))` to keep expansion sets aligned with catalog reality.
...
- `apps/mac/touch-code/Tests/HierarchySidebarFeatureTests.swift` — 4 tests: toggleSpaceExpansion, toggleProjectExpansion, worktreeRowTappedForwardsToHierarchyClient (LockIsolated closure captures arguments), pruneExpansionSetsDropsStaleIDs.
...
**What landed:**
- `apps/mac/touch-code/App/Clients/TerminalClient.swift` — gains `surface(paneID)` closure for registry lookup after `ensureSurface`. `live(engine:)` forwards to `engine.ghosttyRuntime?.surface(for:)`; `testValue` uses `unimplemented(…, placeholder: nil)`.
...
Add `apps/mac/touch-code/Tests/RootFeatureTests.swift` with two tests: `onLaunchArmsEventSubscription` (uses a `TestStore` with `terminalClient.events` yielding a single fake `.paneReady` then finishing, asserts the `.engineEvent` action is received) and `onLaunchCancelsOnFinish` (asserts no leak after the stream ends).
...
Add `apps/mac/touch-code/Tests/SplitViewportLazyLifecycleTests.swift`: seed a Tab with two Panes; simulate `.tabActivated`; assert `ensureSurface` was called twice (once per Pane, via a recording `TerminalClient.testValue`). Tab deactivation test: assert `closeSurface` was **not** called.
