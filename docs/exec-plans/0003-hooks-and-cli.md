...
- [ ] M2.1 — Hot-path internals + ProcessHookExecutor + full EventMapper (**consumer impact:** exec-plan 0006 C6 M7 is blocked on `HookDispatcher.attach(to:)` actually mapping `TerminalEvent → HookEnvelope` end-to-end; ship as the first post-close follow-up)
...
- [ ] M8.1 — Real-socket (`UnixSocketTransport` + `SocketServer`) integration tests + completion scripts + `tc --man` + **coverage gaps**: `tc send`, `tc broadcast`, `tc tab activate`, `tc worktree activate`, oversize-frame rejection, `LOCAL_PEERCRED` auth once M3.1 lands
