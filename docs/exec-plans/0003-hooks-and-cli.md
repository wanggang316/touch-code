...
- [x] M7 — `tc open [--in EDITOR]` + `ExternalEditor` app-side service (resolves product-spec Q7) — 2026-04-20 (ExternalEditor with 6-editor allowlist (vscode/cursor/zed/xcode/subl/finder), ProcessRunner protocol for test isolation, fallback to `finder` when neither caller nor `Project.defaultEditor` picks one; SystemOpenHandlers wraps `system.openInEditor` + `system.openPath` RPCs with error mapping to CLIExitCode-correct IPCError variants; `tc open [--in EDITOR] [--path PATH] [<worktree>]` subcommand routes through AliasResolver. 10 new ExternalEditorTests + all prior suites: tcKit 29/6 + touch-code 84/16 + TouchCodeCore 88/14 green; lint clean. `Task.detached` + `nonisolated` ProcessRunner keeps blocking `Process.waitUntilExit()` off MainActor.)
