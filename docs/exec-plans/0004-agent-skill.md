...
- **DEC-17 (M6 polish, 2026-04-20): `skill-help-roundtrip.py` only inspects code-scope markdown.** Earlier regex matched `tc foo` in any prose — including "run tc foo first" with no backticks — which would flag benign explanatory prose as CLI contract claims once M8 fills in content. Now the checker only scans fenced ``` blocks and inline `…` spans; prose outside backticks is deliberately ignored. A real CLI claim is always in code font by convention in this skill.
- **DEC-18 (M6 polish, 2026-04-20): `skill-orthogonality-check.sh` allowlist is anchored to the file-path column.** Prior pattern `grep -v -E 'SkillInstaller.swift|…'` matched any line containing those basenames anywhere — so a new `SkillCommand.swift` planted in `TouchCodeCore/` would silently exempt itself. The pattern is now `(^|/)(<basename>):[0-9]+:`, which only matches the basename at the end of a path component immediately before the `:line:` separator. The invariant is load-bearing; the regex now matches that load-bearing status.
