...
5. **Agent detection heuristic** — *Resolved by exec-plan 0003 (C3 D10 + C4 Panel labels):* no heuristic detection. C6 relies on (a) user-configured hook rules + (b) Panel labels applied at spawn time via `tc panel label @panel <label>`. Known-binary allowlists are out of scope — they collide with user-renamed binaries and shell aliases, and the wrapper-composition story is stronger without "magic." See [C3 design doc §D10](design-docs/c3-lifecycle-hooks.md).
...
7. **External editor discovery & invocation (C8)** — *Contract resolved by exec-plan 0003 (C4 D14); app-side implementation ships with exec-plan 0005 (C8).* Built-in allowlist of 6 IDs (`vscode`, `cursor`, `zed`, `xcode`, `sublime`, `finder`) with fixed argv templates; per-Project override via `Project.defaultEditor`; user templates via `settings.json.externalEditors[NAME]`. The `tc open [--in EDITOR] [<worktree>]` CLI wrapper landed in 0003 M7 and calls the `editor.open` RPC; the server-side `EditorService` + handler land in 0005 M5–M7. See [C4 design doc §D14](design-docs/c4-cli.md) and [exec-plan 0005](exec-plans/).
