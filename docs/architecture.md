...
| `touch-code` | macOS app | `apps/mac/touch-code/{App,Runtime,Hooks,Git}/` | The Mac app. Buildable subfolders compile as one target. Depends on `TouchCodeCore`, `TouchCodeIPC`, `tc`; the `tc` binary is embedded inside the app bundle at `Touch Code.app/Contents/Resources/bin/tc` via the `Embed tc` post-script (`apps/mac/scripts/embed-tc.sh`), giving the c4-cli first-launch installer a stable symlink target. |
