...
- `TouchCodeCore` (domain types: `Pane`, `Tab`, `Worktree`, `PaneID`, …) — leaf package, zero internal deps.
- `Runtime` (in-app module under `apps/mac/touch-code/Runtime/`) — owns `GhosttyRuntime`, `PaneSurface`, `TerminalEngine`, `HierarchyManager`, `CatalogStore`. Exposes `AsyncStream<TerminalEvent>` per [the C1+C2 design](0001-terminal-and-hierarchy.md) and [exec plan 0002](../exec-plans/0002-terminal-and-hierarchy.md).
...
public nonisolated struct HookSubscription: Codable, Equatable, Sendable, Identifiable {
  ...
  public enum Scope: Codable, Equatable, Sendable {
    case anyPanel
    case paneID(PaneID)
    case panelLabel(String)           // set via `tc label <pane> --tag foo`
    case tabID(TabID)
...
4. **Executor.** `ProcessHookExecutor` wraps `Foundation.Process`:
   ...
   - `environment`: user env **plus** reserved keys `TOUCH_CODE_SOCKET_PATH`, `TOUCH_CODE_EVENT`, `TOUCH_CODE_VERSION`, and the appropriate `TOUCH_CODE_*_ID` for whichever anchors are present in the envelope. `TOUCH_CODE_PANE_ID` is set iff `envelope.pane != nil`, etc.
...
Every macOS app can already expose itself to AppleScript. Users would write `tell application "touch-code" to send text "ls" to pane id "…"`.
...
- Handler **stdin is trusted app-produced JSON**, not user-typed text, so shell-injection via `command` strings is the handler author's responsibility. Example in docs warns that `command: "notify '$PANEL_ID'"` is unsafe; correct form is `command: "notify \"$TOUCH_CODE_PANE_ID\""`.
...
- **R9 — Users write handlers that assume `pwd` is the Worktree root, but the dispatcher resolves `cwd` to `subscription.cwd ?? envelope.anchorPath`.** For Pane-scoped events, anchor is the Pane's `workingDirectory`, which could differ from the Worktree path.
  - *Mitigation:* document clearly; `cwd: null` is the default "pane's pwd"; `cwd: "$WORKTREE"` (expanded from `TOUCH_CODE_WORKTREE_PATH` env var) is the obvious override.
