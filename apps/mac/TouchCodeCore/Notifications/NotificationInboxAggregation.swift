...
extension NotificationInbox {
  ...
  /// Returns the number of unread, non-dismissed notifications whose pane
  /// resolves to the given Worktree. Panes that no longer belong to any
  ...
  public func unreadCount(forWorktree worktreeID: WorktreeID, in catalog: Catalog) -> Int {
    ...
    return notifications.reduce(into: 0) { total, notification in
      ...
      guard index[notification.paneID] == worktreeID else { return }
      total += 1
    }
  }
  ...
  /// Render-hot paths should cache a snapshot-scoped index.
  /// Counts unread, non-dismissed notifications whose pane resolves to
  ...
  /// `paneID` no longer resolves to any Worktree — are excluded so the
  /// badge count and the popover grouping derived from
  /// `paneWorktreeIndex()` agree on the visible row count.
  public func totalUnread(in catalog: Catalog) -> Int {
    let index = catalog.paneWorktreeIndex()
    return notifications.reduce(into: 0) { total, notification in
      guard notification.isUnread else { return }
      guard index[notification.paneID] != nil else { return }
      total += 1
    }
  }

  /// Render-hot paths should cache a snapshot-scoped index.
  /// Every notification whose pane resolves to the given Worktree, sorted
  ...
  public func notifications(forWorktree worktreeID: WorktreeID, in catalog: Catalog) -> [AgentNotification] {
    ...
    return
      notifications
      .filter { index[$0.paneID] == worktreeID }
      .sorted { lhs, rhs in
...
extension Catalog {
  /// `[PaneID: WorktreeID]` over every pane in the catalog. Built from one
