...
3. **Ambient** — omit the target entirely; `tc` resolves it from `$TOUCH_CODE_PANEL_ID`
   / `$TOUCH_CODE_TAB_ID` / `$TOUCH_CODE_WORKTREE_ID` in the invocation's environment.

## Ambient targeting

When a command runs inside a touch-code Panel, most subcommands can omit the target:

```bash
tc panel focus        # focuses the current Panel
...
tc panel split right  # splits the current Panel
```
...
tc panel focus 1/2/1/3/2
tc panel focus <panel-uuid>
```
...
| `tc panel focus / close / capture / resize / notify` | Panel selector or UUID |
| `tc panel split` | Tab or Panel selector/UUID (via `--in`) |
| `tc panel send` | optional Panel selector or UUID (first arg; stdin fallback) |
| `tc send` / `tc broadcast` | Panel or Tab selector/UUID |
