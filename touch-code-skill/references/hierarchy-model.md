...
└── Project (= a git repo)
    └── Worktree (= a `git worktree` with its own directory + branch)
        └── Tab
            └── Panel (= a libghostty terminal session)
```
...
Panel is a single shell; multiple Panels per Tab form split layouts.
...
Every Panel inherits three environment variables that let `tc` know *where* a command
was issued from, so most invocations can omit the target argument:

- `TOUCH_CODE_PANEL_ID` — UUID of the current Panel.
...
these manually — touch-code sets them before handing the Panel to the user's shell.
...
- **A Panel always belongs to exactly one Tab.** Closing a Panel never orphans it.
